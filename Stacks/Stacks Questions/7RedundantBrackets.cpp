...
bool findRedudant(string &s)
{
    ...
    for (int i = 0; i < s.length(); i++)
    {
        ...
        else
        {
            ...
            if (ch == ')') // closing brakcet mila to to ab stack me check karo for operators till we find a opening bracket to determine reductant or not
            {
                ...
                while (st.top() != '(')
                {
                    ...
                    {
                        isRedudant = false; // not faltu bracket
                    }
